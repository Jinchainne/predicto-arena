...
async function main() {
  ...
  const contractPath = path.resolve(process.cwd(), "contracts/predicto_arena.py");
  ...
  console.log("Verified get_total_markets:", totalMarkets);
  ...
  writeFileSync(path.resolve(process.cwd(), "frontend/.env.local"), `${envLines.join("\n")}\n`);
